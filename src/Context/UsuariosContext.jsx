...
//function de CADASTRO DE USUARIO
  function onSubmitUser(cadFormUser) {
    const verificacao = listUsuarios.find((user) => (user.email === cadFormUser.email) || (user.cpf === cadFormUser.cpf))
    if(verificacao) {
      alert('Usuário já cadastrado!')
      return
    }
    const potato = {
      ...cadFormUser, isLogado: false
    }
...
async  function loginUser(formLogin) {
    try{
     const userLog = listUsuarios.find((user) => (user.email == formLogin.email) && (user.password == formLogin.password ))
  ...
  async function logout() {
    ...
    const res = await fetch(`http://localhost:3000/listUsuarios/${userLogout}`)
    ...
    console.error(error)
        ...
        return(
            ...
            <UsuariosContext.Provider value={{listUsuarios, onSubmitUser, loginUser, logout}}>
